#В этом примере определяем класс доставки, который использует определенную стратегию для расчета стоимости доставки
#на основе веса посылки.
#Класс Shipping принимает объект strategy в своем конструкторе и делегирует ему вычисление стоимости 
#доставки с использованием метода calculate.

#Определяем интерфейс базовой стратегии доставки с помощью единого метода calculate, который реализуется 
#конкретными стратегиями FlatRateShipping, FreeShipping и WeightBasedShipping.
#Каждая стратегия рассчитывает стоимость доставки на основе своей собственной логики.

#Создаем новый экземпляр Shipping с определенной стратегией и используем его для расчета стоимости доставки
#посылки весом 10. 
